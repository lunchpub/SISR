...
# Run this command
# python process_1_data.py --data-root data/VSAIv1/split_ss_444_lsv
...
def main_extract_subimages(args):
    ...
    Usage:
        ...
        Typically, there are four folders to be processed for VSAI dataset.
            train
            train_LR_bicubic/X2
            train_LR_bicubic/X3
            train_LR_bicubic/X4
    ...
    # HR images
    opt['input_folder'] = osp.join(args.data_root, 'train/images')
    opt['save_folder'] = osp.join(args.data_root, 'train_HR_sub')
    ...
    for scale in [4]:
        opt['input_folder'] = osp.join(args.data_root,
                                       f'train_LR_bicubic/X{scale}')
        opt['save_folder'] = osp.join(args.data_root,
                                      f'train_LR_bicubic/X{scale}_sub')
...
def worker(path, opt):
    ...
    # remove the x2, x3, x4 and x8 in the filename for VSAI
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Prepare VSAI dataset',
